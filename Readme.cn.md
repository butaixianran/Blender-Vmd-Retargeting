#### 本扩展现在运行良好，几乎处理所有vmd动作都没有问题。最近2个月的所有用户反馈，最终发现，全部是用户要么没有按照教程来，要么基本不会用Blender。因此，如果你导入有问题，请先对着本文最后的"[常见问题](#常见问题)"部分，自查哪里没有弄对。   
...
如果直接导入这种vmd文件到Daz或CC人模，会出现像机器舞一样，从一个姿势跳到另一个姿势的现象。  
...
CC和Daz模型默认没有IK。因此本扩展在从vmd文件导入身体运动时，会创建IK  

但是，Daz/CC人模有个问题：膝盖的默认姿势下完全没有弯曲。于是，就需要有个IK Pole骨骼，来告诉膝盖弯曲时，该指向哪个方向。  

因此，Daz/CC的膝盖，会一直指向它们前面的IK Pole骨骼。而IK Pole骨骼会跟随盆骨旋转。  

这个机制在大部分时候工作良好。但是，如果腿部旋转过大，那么，只是跟随盆骨的IK Pole骨骼就不太行了。    

这种情况下，你需要从一个mmd模型上转换动作数据。因为mmd模型上已经计算过IK了，所以Daz/CC人模可以直接获取腿部的最终旋转，而无须再创建IK。  
...
# 常见问题
## 动作导入后脚不弯曲
该动作没有使用IK，因此导入时请去掉勾选IK。  

去掉IK后，如果动作正常，但脚下有滑步现象，请在扩展面板，把x轴位移比例设为0.5。  
...
## 人模不动，提示：Selected Object is not an ARMATURE
这句话的意思是，你选择的物体不是骨架。请在大纲视图中，选择骨架，而不是在3D预览区选择人模表面。  
